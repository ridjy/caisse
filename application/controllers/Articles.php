...
class Articles extends CI_Controller {
	...
	public function getAllArticles()
	{
		...
		if(!empty($rResult)){ 
		 	foreach ($rResult as $item) {
		 	 	...
		 	 	$row[] = "<span title='Imprimer' class='cpointer text-underline' onclick='ShowCB(this)' data-barcode='".$item->codebarre_article."' data-nom_article='".$item->nom_article."' data-uniqid='".$item->uniqid."' data-taille='".$item->nom_taille."'>".$item->codebarre_article."</span>" ;
	...
	public function showmodal()
	{
		...
		if (!empty($getAllSize)) {
			$optSize .= "<option value='0'>Selectionner une taille</option>" ;
		...
		else
		{
			...
			$htmlWrapper .= 	"<div class='col-md-4'>" ;
			...
			$htmlWrapper .= 	"</select>" ;
			$htmlWrapper .= "</div>" ;
			$htmlWrapper .= "<div class='form-group ".$hidden." '>" ;
