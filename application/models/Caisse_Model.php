...
class Caisse_Model extends CI_Model {
	...
	public function Get_All_Facture($facture_id)
	{
		...
		$fact = $this->Global->selectRow('factures','facture_numero,mode_paiement,facture_montant,montant_recu,rr','id_facture='.$facture_id) ;
		$modePaiement = $this->Global->selectRow('mode_paiement','type','id_mp='.$fact->mode_paiement) ;

        if(!empty($data)){
        	$res['facture']['facture_numero'] = $fact->facture_numero ;
        	$res['facture']['id_mode_paiement'] = $fact->mode_paiement ;
        	$res['facture']['mode_paiement'] = $modePaiement->type ;
        	$res['facture']['facture_montant'] = $fact->facture_montant ;
        	$res['facture']['montant_recu'] = $fact->montant_recu ;
        	$res['facture']['rr'] = $fact->rr ;
